...
const priorities = {
  ...
  Média: { dot: "bg-ebot-orange", label: "text-ebot-orange", chip: "bg-ebot-orange/[0.10] border-ebot-orange/25" },
  Baixa: { dot: "bg-ebot-teal", label: "text-ebot-teal", chip: "bg-ebot-teal/[0.10] border-ebot-teal/25" }
} as const;
...
export function PriorityBadge({ priority, withChip = false }: { priority: "Alta" | "Média" | "Baixa"; withChip?: boolean }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-[11px] font-extrabold text-ebot-muted">
      <span className={cn("size-1.5 rounded-full", config.dot)} />
      {priority}
    </span>
  );
