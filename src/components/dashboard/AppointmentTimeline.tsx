...
export function AppointmentTimeline() {
  ...
  return (
    <GlassCard data-right-panel className="h-full min-w-0 overflow-hidden p-4 sm:p-5 lg:p-6">
      ...
      <div className="clinical-scrollbar mt-5 flex max-w-full gap-2 overflow-x-auto pb-1">
        ...
        {visibleAppointments.length ? visibleAppointments.map((appointment) => {
          ...
          return (
            <div key={`${appointment.time}-${appointment.title}`} className="relative grid min-w-0 grid-cols-[48px_minmax(0,1fr)] gap-2.5 sm:grid-cols-[58px_minmax(0,1fr)] sm:gap-3">
              <div className="pt-4 text-[13px] font-extrabold text-clinical-muted sm:text-[14px]">{appointment.time}</div>
              <div className="relative min-w-0 rounded-[22px] border border-clinical-border/[0.10] bg-clinical-surface/72 p-3.5 transition hover:-translate-y-0.5 hover:border-clinical-blue/25 hover:bg-clinical-surface">
