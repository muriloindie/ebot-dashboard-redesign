...
export function ContactReasons() {
  return (
    <GlassCard className="p-5 lg:p-6">
      <h3 className="text-xl font-extrabold tracking-[-0.03em] text-clinical-dark">Motivos de contato</h3>
      <p className="mt-2 text-sm leading-6 text-clinical-muted">Ranking dos principais motivos de demanda operacional.</p>
      <div className="mt-6 space-y-4">
        {contactReasons.map((reason, index) => (
          <div key={reason.label}>
            <div className="mb-2 flex items-center justify-between gap-3 text-sm">
              <div className="flex min-w-0 items-center gap-2 font-bold text-clinical-dark">
                <span className="flex size-6 shrink-0 items-center justify-center rounded-full bg-clinical-blue/10 text-[11px] text-clinical-blue">{index + 1}</span>
